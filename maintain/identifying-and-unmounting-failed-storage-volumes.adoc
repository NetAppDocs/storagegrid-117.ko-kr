...
. 볼륨이 사용 중이어서 마운트 해제에 실패하면 를 사용하여 마운트 해제를 강제 실행할 수 있습니다 `--use-umountof` 옵션:
+

NOTE: 를 사용하여 마운트 해제 강제 실행 `--use-umountof` 옵션을 사용하면 볼륨을 사용하는 프로세스 또는 서비스가 예기치 않게 동작하거나 충돌될 수 있습니다.
